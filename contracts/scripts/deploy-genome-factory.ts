/**
 * Deploy Factory (Genome Protocol) to OPNet testnet
 *
 * Run: npx ts-node scripts/deploy-genome-factory.ts
 *
 * Prerequisites:
 *  - .env with EC_PRIVATE_KEY (64 hex) and MLDSA_PRIVATE_KEY (5120 hex) set
 *  - build/Factory.wasm must exist (run: npm run build:factory)
...
console.log('=== Factory Deployment (Genome Protocol) ===');
...
const wasmPath   = join(__dirname, '..', 'build', 'Factory.wasm');
...
// ---------------------------------------------------------------------------
// Sign deployment (no calldata — Factory.onDeployment only reads tx.sender)
// ---------------------------------------------------------------------------
console.log('Signing deployment...');
const txFactory = new TransactionFactory();
const result    = await txFactory.signDeployment({
    bytecode,
    challenge,
...
console.log('Contract address:', result.contractAddress);
console.log('Contract pubkey :', result.contractPubKey);
console.log('\nSave the contract address — update frontend/src/config.ts with the new factory address and pubkey.');
