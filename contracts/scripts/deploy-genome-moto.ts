...
if (!UNDERLYING_ADDRESS) throw new Error('MOTO_UNDERLYING not set in .env');

// gToken naming convention: g prefix required
const GENOME_DECIMALS  = 18;
const GENOME_NAME      = 'gMoto';
const GENOME_SYMBOL    = 'gMOTO';
...
console.log('=== gMoto Genome Deployment ===');
console.log('Underlying:', UNDERLYING_ADDRESS);
console.log('Name      :', GENOME_NAME, '/', GENOME_SYMBOL);

const writer = new BinaryWriter();
writer.writeAddress(Address.fromString(UNDERLYING_ADDRESS));
writer.writeU8(GENOME_DECIMALS);
writer.writeStringWithLength(GENOME_NAME);
writer.writeStringWithLength(GENOME_SYMBOL);
...
const bytecode  = new Uint8Array(readFileSync(join(__dirname, '..', 'build', 'Genome.wasm')));
...
const result    = await txFactory.signDeployment({ bytecode, calldata, challenge, signer: wallet.keypair, mldsaSigner: wallet.mldsaKeypair, network: NETWORK, utxos, from: opnetP2TR!, feeRate: FEE_RATE, priorityFee: PRIORITY_FEE, gasSatFee: GAS_SAT_FEE });
...
console.log('\n=== Done ===');
console.log('gMoto Genome address:', result.contractAddress);
console.log('gMoto Genome pubkey :', result.contractPubKey);
console.log('\n>> Add to .env: GENOME_ADDRESS=' + result.contractPubKey);
