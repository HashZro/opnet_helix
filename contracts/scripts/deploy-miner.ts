/**
 * Deploy MinerToken to OPNet testnet
 *
 * Run: npm run deploy:miner
 *
 * Prerequisites:
 *  - .env with EC_PRIVATE_KEY (64 hex) and MLDSA_PRIVATE_KEY (5120 hex) set
 *  - build/MinerToken.wasm must exist (run: npm run build:miner)
...
console.log('=== MinerToken Deployment ===');
...
const wasmPath   = join(__dirname, '..', 'build', 'MinerToken.wasm');
