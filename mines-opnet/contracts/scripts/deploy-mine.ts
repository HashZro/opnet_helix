/**
 * Deploy Mine (xToken vault) to OPNet testnet
 *
 * Run: npm run deploy:mine
 *
 * Prerequisites:
 *  - .env with EC_PRIVATE_KEY (64 hex) and MLDSA_PRIVATE_KEY (5120 hex) set
 *  - build/Mine.wasm must exist (run: npm run build:mine)
 *  - Deployer wallet must have testnet BTC for fees
 *  - UNDERLYING_ADDRESS must point to a deployed OP_20 token
 */
...
const wasmPath   = join(__dirname, '..', 'build', 'Mine.wasm');
...
// ---------------------------------------------------------------------------
// Sign deployment (with calldata for Mine constructor params)
// ---------------------------------------------------------------------------
