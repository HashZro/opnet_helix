/**
 * Deploy Staking contract to OPNet testnet
 *
 * Run: npm run deploy:staking
 *
 * Prerequisites:
 *  - .env with EC_PRIVATE_KEY (64 hex) and MLDSA_PRIVATE_KEY (5120 hex) set
 *  - build/Staking.wasm must exist (run: npm run build:staking)
 *  - Deployer wallet must have testnet BTC for fees
 *  - XMINER_ADDRESS must point to a deployed Mine (xToken) contract
 */
...
// Matches Staking.onDeployment(): xMiner (Address)
// ---------------------------------------------------------------------------
const XMINER_ADDRESS = process.env.XMINER_ADDRESS
...
writer.writeAddress(Address.fromString(XMINER_ADDRESS));

const calldata = writer.getBuffer();
console.log(`Calldata        : ${calldata.length} bytes`);
console.log(`xMiner address  : ${XMINER_ADDRESS}`);

// ---------------------------------------------------------------------------
// Load compiled WASM binary
// ---------------------------------------------------------------------------
const __dirname  = dirname(fileURLToPath(import.meta.url));
const wasmPath   = join(__dirname, '..', 'build', 'Staking.wasm');
...
// ---------------------------------------------------------------------------
// Sign deployment (with calldata for Staking constructor params)
// ---------------------------------------------------------------------------
