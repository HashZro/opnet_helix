#!/bin/bash
# ralph.sh — Run Claude in a Ralph Wiggum loop for Mines Protocol
...
# ---------------------------------------------------------------------------
# Helper: get model for next story (reads "model" field from prd.json)
# Falls back to CLI arg $MODEL if story has no model field
# ---------------------------------------------------------------------------
next_story_model() {
  local story_model
  story_model=$(jq -r '[.stories[] | select(.passes == false)] | sort_by(.priority) | .[0] | .model // empty' prd.json 2>/dev/null)
  if [ -n "$story_model" ] && [ "$story_model" != "null" ]; then
    echo "$story_model"
  else
    echo "$MODEL"
  fi
...
echo ""
echo "========================================"
echo "  Mines Protocol Ralph Loop"
echo "========================================"
echo "  Model:      $MODEL"
...
for ((i=1; i<=ITERATIONS; i++)); do
  ...
  ITER_MODEL=$(next_story_model)
  echo ""
  echo "========================================"
  echo "  Iteration $i of $ITERATIONS"
  echo "  Remaining: $REMAINING stories"
  echo "  Next:      $NEXT"
  echo "  Model:     $ITER_MODEL"
  ...
  for ((retry=1; retry<=MAX_RETRIES; retry++)); do
    ...
    run_with_timeout "$ITERATION_TIMEOUT" \
      claude --dangerously-skip-permissions --model "$ITER_MODEL" -p \
"You are working on the Mines Protocol OPNet project.
